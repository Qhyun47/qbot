...
ER Scribe는 **응급실에서 환자를 문진하는 응급의학과 의사**를 위한 **모바일 우선 차팅 어시스턴트**로 다음 기능을 제공합니다:
...
- ✅ **Task 001: 프로젝트 구조 및 라우팅 설정** - 완료 (2026-04-14)
  ...
  - 기존 `app/protected/` 스타터 샘플은 유지 (건드리지 않음)
...
- **Task 003: AI 리소스 폴더 및 Placeholder 파일 생성**
  - `lib/ai/resources/` 폴더 골격 생성
  - `style_guide.md`, `structured_schema.json`, `cc-list.json` placeholder 파일 생성
  - `lib/ai/resources/templates/` 폴더 + 대표 C.C 템플릿 placeholder (chest-pain.json 등)
  - `lib/ai/resources/examples/` 폴더 + 빈 `.jsonl` 파일
  - `lib/ai/resources/guides/` 폴더 + 대표 C.C 시스템 기본 가이드라인 placeholder (Markdown)
  - `lib/ai/load-resources.ts` 명시적 매핑 함수 스텁 생성
...
- **Task 004: 공통 컴포넌트 및 더미 데이터 유틸리티** - 우선순위
...
- **Task 006: 새 케이스 입력 페이지 UI (분할 화면 포함)**
  - `/cases/new` 페이지 UI 완성
...
- **Task 007: 케이스 결과 페이지 UI (데스크탑 우선)**
...
- **Task 008: 케이스 목록·가이드라인 관리·설정 페이지 UI**
...
- **Task 009: Supabase 데이터베이스 및 RLS 구축** - 우선순위
...
- **Task 010: AI 파이프라인 CLI 하네스 구축 및 품질 검증** ⭐ - 우선순위
  - `lib/ai/gemini-client.ts` — `@google/genai` SDK 래퍼
  ...
  - `lib/ai/generate-template.ts` — Stage 3: JSON + 템플릿 + few-shot → 채워진 상용구
  - `lib/ai/load-resources.ts` — C.C별 template/example/guide 로딩 (명시적 맵)
  - `scripts/ai-harness.ts` — CLI 하네스: `npx tsx scripts/ai-harness.ts fixtures/case-01.json`
  - `fixtures/` 폴더에 실제 ER 케이스 2~3개 JSON (사용자 제공 필요)
  - `.env.local`에 `GOOGLE_GENAI_API_KEY` 설정
  - **품질 검증 수행 후 프롬프트/few-shot 튜닝 반복**
  - **테스트 체크리스트**:
    ...
    - [ ] Stage 3 상용구의 각 필드가 적절히 채워짐 (빈 필드 최소화)
    - [ ] 의사 검수자가 "쓸만한 초안" 판정 (만족 전까지 튜닝 반복)

- **Task 011: AI 생성 API Route 및 폴링 구현**
  - `app/api/cases/[id]/generate/route.ts` POST 핸들러 완성
    - `export const runtime = 'nodejs'`, `export const maxDuration = 60`
    - 즉시 `status='generating'` 업데이트
    - Task 010의 순수 함수 3개 순차 호출
    - 결과를 `case_results`에 insert, `cases.current_result_id` 업데이트
    - 부분 성공 허용 (Stage 2 실패 시 Stage 3는 시도)
    - 에러 처리 및 `error_message` 기록
  - `app/api/cases/[id]/status/route.ts` GET 핸들러 (폴링 전용 경량 엔드포인트)
  - `components/cases/generation-poller.tsx` 클라이언트 컴포넌트 (3~5초 폴링, 완료 시 `router.refresh()`)
...
- **Task 012: 케이스 CRUD Server Actions 및 DB 연동**
...
- **Task 013: 가이드라인 관리 및 레이아웃 설정 기능 구현**
...
## 🚨 Phase 3 진입 전 필수 확인 사항 (사용자 제공 필요)

Phase 3의 Task 010(AI 품질 검증)은 아래 리소스 없이는 시작할 수 없습니다:

1. **익명화된 실제 ER 케이스 샘플 2~3개** (fixture JSON 기반 품질 검증용)
2. **Gemini API 키** (`GOOGLE_GENAI_API_KEY`)
3. **차트 출력 언어 확정** (한국어 / 영어 / 혼용)
4. **`cc-list.json` 실제 목록** (KTAS 기준인지 병원 내부 리스트인지)
5. **상용구 `template.json` 샘플 1개** (필드 기반 / Markdown / 문장 블록 중 어느 형식인지)
6. **`structured_schema.json` 초안** (ChatGPT로 1차 정리 예정)
7. **Vercel Pro 플랜 사용 가능 여부** (60초 함수 한도)

위 항목은 Phase 1·2 진행 중 병행 준비해도 괜찮지만, **Phase 3 Task 010 착수 시점에는 반드시 준비되어 있어야 합니다.**
