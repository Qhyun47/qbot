...
응급실 의사의 모바일 문진 정보를 AI 차팅 초안으로 변환해주는 어시스턴트 웹 서비스입니다.
...
- ✅ **Task 014: PWA 설치 및 오프라인 대응** - 완료 (2026-04-22)
  - `public/manifest.json` 작성 (name: 규봇, `start_url: "/"`, `display: "standalone"`, `theme_color: #09090b`)
  - `public/icons/` SVG 아이콘 생성 (icon.svg, icon-maskable.svg) + manifest에 `image/svg+xml`로 등록
  - 수동 Service Worker (`public/sw.js`) — network-first 전략, 오프라인 시 `/offline` 반환
  - 오프라인 기본 페이지 (`app/offline/page.tsx`) — Wifi off 아이콘 + "네트워크 연결이 필요합니다" + [다시 시도] 버튼
  - `lib/pwa/is-standalone.ts` — `window.matchMedia` + `navigator.standalone` 체크
  - `components/pwa/sw-register.tsx` — `useEffect`로 `/sw.js` 등록 클라이언트 컴포넌트
  - `app/layout.tsx` metadata에 `manifest`, `themeColor`, `appleWebApp`, `icons.apple` 추가 + `<SwRegister />` 삽입
  - `components/pwa/install-banner.tsx` — Android(`beforeinstallprompt`) / iOS Safari(userAgent 감지) 분기 설치 유도 배너, `sessionStorage` dismissed 플래그
  - `app/page.tsx`에 `<InstallBanner />` 삽입
  - **테스트 체크리스트**:
    - [ ] Chrome Lighthouse PWA 점수 90+ 달성 (HTTPS 배포 후 확인 필요)
    - [ ] 안드로이드 Chrome에서 "홈 화면에 추가" 동작 확인 (실기기 테스트 필요)
    - [ ] iOS Safari에서 홈 화면 아이콘 탭 시 standalone 실행 확인 (실기기 테스트 필요)
    - ✅ 오프라인 페이지(`/offline`) 렌더링 정상 — "네트워크 연결이 필요합니다" + [다시 시도] 버튼 표시 확인
    - ✅ localhost에서 Service Worker `/sw.js` 등록 및 `activated` 상태 확인
    - ✅ `manifest.json` 200 응답 및 파싱 정상 (`name: 규봇`, `display: standalone`)
    - ✅ `theme-color: #09090b`, `apple-mobile-web-app-title: 규봇` 메타태그 정상 삽입 확인
    - ✅ 미들웨어 수정: `manifest.json`, `sw.js`, `icons/`, `/offline` 인증 우회 처리
...
- **Task 015: 성능 최적화 및 배포**
  ...
  - Vercel 배포 설정 (환경 변수, HTTPS)
  - Vercel Hobby(Free) 플랜 사용 — AI 파이프라인 병렬화(Promise.all)로 10초 한도 내 처리
...
7. ✅ **Vercel Hobby(Free) 플랜 사용** — AI 파이프라인 병렬화(Task 2a2d)로 10초 한도 내 처리 가능 확인
