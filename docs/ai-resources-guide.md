...
```
C.C. (cc-list.json)
  ├── guideKeys[] → 가이드라인 파일 (ai-docs/guides/{guideKey}/guide.html)
  └── templateKeys[] → 상용구 파일 (ai-docs/templates/{templateKey}/template.json + schema.json)
                            └── 케이스 예시 데이터 (ai-docs/templates/{templateKey}/examples.md)
```
...
| `ai-docs/guides/{guideKey}/guide.html`          | 가이드라인 HTML (HWP→HTML, processGuideHtml() 적용)         |
| `ai-docs/templates/{templateKey}/template.json` | 상용구 템플릿 (fields, pe, history 포함)                    |
| `ai-docs/templates/{templateKey}/schema.json`   | AI 정규화 스키마 (JSON Schema draft-07)                     |
...
4. 사용자에게 HWP에서 내보낸 HTML을 요청 → `processGuideHtml()` 적용 후 `ai-docs/guides/{key}/guide.html` 저장
...
> 파일은 `ai-docs/guides/{key}/guide.html`로 저장됩니다.
...
4. 파일 2개 생성:
   - `ai-docs/templates/{key}/template.json`
   - `ai-docs/templates/{key}/schema.json`
...
실제 예시: `ai-docs/templates/chest-pain/template.json` 참조
