...
```
C.C. (cc-list.json)
  ├── guideKeys[] → 가이드라인 파일 (ai-docs/cc/{key}/guide.md)
...
| `ai-docs/cc/{key}/guide.md`           | C.C.별 문진 가이드라인 Markdown                         |
...
4. `ai-docs/cc/{key}/guide.md` 파일 생성 (Claude가 초안 작성)
...
실제 예시: `ai-docs/cc/chest-pain/guide.md` 참조

### 준비물

가이드라인 내용을 직접 제공하거나, 제공하지 않으면 Claude가 의학 지식 기반으로 초안을 작성합니다.
**의학 내용은 반드시 직접 검토·수정해야 합니다.**
