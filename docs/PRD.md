...
**목적**: 응급실 의사가 모바일로 문진 키워드를 입력하면 AI가 차팅 초안 2종(HPI 줄글 + C.C 상용구)을 자동 생성해주는 어시스턴트
...
| **F004** | AI 차팅 생성             | 입력 카드 → 3단계(정규화→HPI→상용구) 처리 → 초안 2종 생성                   |
| **F005** | 차팅 편집 및 복사        | HPI/상용구 편집 + 독립 복사 버튼                                            |
