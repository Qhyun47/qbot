...
async function run() {
  ...
  // Stage 2
  separator("Stage 2: HPI 생성");
  console.log("처리 중...");
  const rawInputs = fixture.inputs.map((i) => ({
    rawText: i.rawText,
    timeTag: i.timeTag,
  }));
  const hpi = await generateHpi(structuredCase, rawInputs);

  console.log("\n[생성된 HPI]");
  console.log(hpi);

  if (fixture.expectedHpi) {
    console.log("\n[예상 HPI (참고)]");
    console.log(fixture.expectedHpi);
  }
  ...
  if (templateKey) {
    ...
    console.log("처리 중...");
    const template = await generateTemplate(structuredCase, templateKey);
    ...
    console.log(
      `C.C. '${cc}'에 대한 templateKey가 없어 상용구 생성을 건너뜁니다.`
    );
  }

  separator("완료");
  console.log("3단계 파이프라인 완주.");
}
