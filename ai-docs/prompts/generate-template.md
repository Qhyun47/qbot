...
`template.output_example`이 출력 형식의 **유일한 기준**이다. 출력을 시작하기 전 다음 단계를 반드시 따른다.

1. `template.output_example`의 줄을 위에서부터 순서대로 확인한다
2. 그 순서 그대로 줄 하나도 빠트리지 않고 출력한다
3. 정보가 없는 줄: 라벨과 콜론은 그대로 유지하고 값 부분만 비워둔다 — 줄 자체를 삭제하지 않는다
4. `referenceExamples`(예시 파일)에 특정 줄이 빠져 있어도 이는 예시의 오류다. 절대 따라 하지 않는다
...
- 해당 정보가 없으면 반드시 빈 문자열("")로 남깁니다
  (라벨과 콜론 줄은 그대로 유지하고, 값 부분만 비워둡니다. 줄 자체를 삭제하지 않습니다)
- 절대 추론하거나 지어내지 않습니다
...
- 현재는 음성(-)이지만 임상적으로 중요한 과거 맥락이 있는 경우, 포맷 출력 뒤에 `: {context}` 형식으로 덧붙입니다
  (예: `Auditory hallucination/Visual hallucination(-/-): 지난 약물 과다복용 시에는 있었음`)
...
- output_example에 있는 줄(라벨 포함)을 정보 없음을 이유로 삭제하거나 생략하는 것
  ($가 없는 줄은 값이 비어있어도 반드시 출력해야 합니다. referenceExamples에 해당 줄이 없어도 마찬가지입니다)
